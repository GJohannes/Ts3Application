...
var privateChatroomUserName = "user" + Math.floor(Math.random() * 40000);
...
function updateButtonsForPersonalMessage(returnData) {
	...
	for (i = 0; i < returnData.allClientNicknames.length; i++) {
		...
		var textOfButton = document.createTextNode("Message to : "
				+ returnData.allClientNicknames[i]);
		...
		document.getElementById('containerWithPersonaleMessageButtons')
				.appendChild(button);
	}
...
function privateMessageButtonOnclick(button) {
	var userName = window.prompt("Please select a name for the Chatroom", privateChatroomUserName);
	if(userName === null){
		return;
	} else {
		privateChatroomUserName = userName;
		window.open(window.location.href + "/privateMessage?User=" + button.value + "&userName=" + userName);		
	}
}